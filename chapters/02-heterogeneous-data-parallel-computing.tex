...
实现彩色转灰度和图像模糊内核函数，本章为简洁起见使用向量加法作为贯穿示例。

\section{向量加法示例}
\label{sec:vector-addition-example}

向量加法可以说是最简单的数据并行计算，相当于顺序编程中 ``Hello World'' 。
在展示向量加法的内核代码之前，先回顾传统向量加法（主机代码）函数的
...
（通常运行在主机上）提供 API 函数，替程序员完成这些活动。下文简称“从主机
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{figs/ch2/2-6.png}
  \caption{管理设备全局内存的 CUDA API 函数}
  \label{fig:2-6}
\end{figure}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{figs/ch2/2-7.png}
...
时才需要拨 1 和区号以及本地号码。CUDA 线程的层次化组织也提供了一种局部性形式，
本书很快就会研究这种局部性。
\end{quote}
