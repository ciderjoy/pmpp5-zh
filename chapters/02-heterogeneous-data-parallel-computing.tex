...
\begin{figure}[htbp]
  ...
  \fbox{%
    \begin{minipage}{0.82\textwidth}
      ...
      \vspace{0.4em}
      \textbf{彩色图像}\quad
      $\xrightarrow{\text{对每个像素独立计算 }L}$
      \quad\textbf{灰度图像}
      \par
      \smallskip
      \small 每个输入像素的 $(r,g,b)$ 三个分量分别参与同一个亮度公式；
      不同像素之间的计算互不依赖。
      \vspace{0.4em}
    \end{minipage}%
  }
  \caption{彩色图像转换为灰度图像的概念示意（依据原书图 2.1 重绘）}
  \label{fig:2-1}
\end{figure}
...
\]
每个元组表示红光（R）、绿光（G）和蓝光（B）的混合。也就是说，渲染像素时，
每个像素的 $r$、$g$ 和 $b$ 值分别表示红、绿、蓝光源的强度，其中 0 表示暗，
1 表示满强度。
\end{quote}
...
\begin{figure}[htbp]
  ...
  \begin{tabular}{c@{\qquad}c@{\qquad}c}
    ...
    $O[0]$ & $O[1]$ & $O[2]$ \\
    \multicolumn{3}{c}{$\cdots$；每一列都可独立执行}
  \end{tabular}
  \caption{彩色转灰度中的数据并行性：像素可以独立计算（依据原书图 2.2 重绘）}
  \label{fig:2-2}
\end{figure}
...
\begin{figure}[htbp]
  ...
  \begin{tabular}{
      ...
      >{\centering\arraybackslash}p{0.18\textwidth}
      >{\centering\arraybackslash}p{0.18\textwidth}}
    & \multicolumn{4}{c}{\textbf{时间向右推进}\hspace{0.6em}$\longrightarrow$}\\[0.3em]
    \textbf{主机（CPU）}
      & \fbox{\parbox[c][1.35cm][c]{0.13\textwidth}{\centering 串行代码}}
      & \fbox{\parbox[c][1.35cm][c]{0.15\textwidth}{\centering 启动 Grid 1\\Kernel A}}
      & \fbox{\parbox[c][1.35cm][c]{0.15\textwidth}{\centering 主机代码继续\\与设备重叠}}
      & \fbox{\parbox[c][1.35cm][c]{0.15\textwidth}{\centering 启动 Grid 2\\并继续执行}}\\[0.5em]
    \textbf{设备（GPU）}
      & \textcolor{gray}{空闲}
      & \multicolumn{2}{c}{
          \fbox{\parbox[c][1.35cm][c]{0.31\textwidth}{\centering
            \textbf{Grid 1：Kernel A}\\
            线程块 0\hspace{0.4em}线程块 1\hspace{0.4em}$\cdots$
            \hspace{0.4em}线程块 $N-1$}}}
      & \fbox{\parbox[c][1.35cm][c]{0.15\textwidth}{\centering
          \textbf{Grid 2：Kernel B}\\多个线程块}}\\
  \end{tabular}
  \endgroup
  \caption{CUDA 程序的执行过程（依据原书图 2.3 重绘）}
  \label{fig:2-3}
\end{figure}
...
实现彩色转灰度和图像模糊内核，本章为简洁起见使用向量加法作为贯穿示例。

\section{向量加法示例}
\label{sec:vector-addition-example}

向量加法可以说是最简单的数据并行计算，是顺序编程中 ``Hello World'' 的并行
...
void vecAdd(float* A_h, float* B_h, float* C_h, int n) {
    ...
    // Part 1: allocate device memory for A, B, and C and copy the inputs
    ...

    // Part 2: invoke the kernel to launch a thread grid on the device
    // and perform the actual vector addition
    ...

    // Part 3: copy C back from the device and free the device vectors
    ...
}
  \end{lstlisting}
  \smallskip
  ...
  \begin{tabular}{
    ...
    \textbf{第 1 部分：准备数据}
      & \textbf{第 2 部分：执行内核}
      & \textbf{第 3 部分：取回结果}\\[0.3em]
    \fbox{\parbox[c][2.7cm][c]{0.245\textwidth}{\centering
      \textbf{主机内存}\\
      $A_h,\ B_h,\ C_h$\\[0.2em]
      $\Downarrow$\ \code{cudaMemcpy}\\[0.2em]
      \textbf{设备全局内存}\\
      分配 $A_d,\ B_d,\ C_d$}}
      &
    \fbox{\parbox[c][2.7cm][c]{0.245\textwidth}{\centering
      \textbf{设备全局内存}\\[0.2em]
      $A_d,\ B_d \xrightarrow{\text{内核}} C_d$\\[0.4em]
      启动线程网格\\
      并行执行向量加法}}
      &
    \fbox{\parbox[c][2.7cm][c]{0.245\textwidth}{\centering
...
并释放不再需要的设备全局内存空间；这些活动对应第 3 部分。CUDA 运行时系统
...
\begin{figure}[htbp]
  ...
  \begin{tabular}{>{\ttfamily}p{0.22\textwidth} p{0.65\textwidth}}
    \toprule
    \code{cudaMalloc()} & 在设备全局内存中分配对象；需要两个参数：指向所分配对象
    的指针地址，以及以字节数表示的分配大小。\\
    \code{cudaFree()} & 从设备全局内存释放对象；需要一个参数，即待释放对象的指针。\\
    \bottomrule
  \end{tabular}
...
\code{cudaMalloc}，为对象分配一段设备全局内存。读者应注意，\code{cudaMalloc}
...
参数。双参数形式使 \code{cudaMalloc} 能像其他 CUDA API 函数一样，使用返回值
报告错误。

\code{cudaFree} 函数与 C \code{free} 函数非常相似。它接收一个输入参数，即
待释放内存的指针，并从设备内存中释放相应空间。
...
地址不应在主机代码中解引用，只能用于调用 API 函数和内核函数。在主机代码中解引用
...
\begin{figure}[htbp]
  ...
  \begin{tabular}{>{\ttfamily}p{0.25\textwidth} p{0.62\textwidth}}
    \toprule
    \code{cudaMemcpy()} & 执行内存数据传送，需要四个参数：目标指针、源指针、
    要复制的字节数，以及传送类型/方向。\\
    \bottomrule
  \end{tabular}
  \caption{在主机与设备之间传送数据的 CUDA API 函数（依据原书图 2.7 重绘）}
  \label{fig:2-7}
\end{figure}
...
分配空间、请求数据传送，并调用真正执行向量加法的内核。我们把这种用于调用内核
...
调试时间；还可以定义 C++ 宏，使源代码中的检查代码更加简洁。原书示例的条件
...
的维度通常反映数据的维度。由于线程是为并行处理数据而创建的，让线程组织方式
反映数据组织方式是自然的。
...
\begin{figure}[htbp]
  ...
  \begin{tabular}{c@{\quad}c@{\quad}c}
    \textbf{块 0} & \textbf{块 1} & \textbf{块 $N-1$} \\
    \texttt{0, 1, 2, \ldots, 254, 255}
      & \texttt{0, 1, 2, \ldots, 254, 255}
      & \texttt{0, 1, 2, \ldots, 254, 255} \\
    \multicolumn{3}{c}{每个线程执行相同的内核代码：}\\
    \multicolumn{3}{c}{\code{i = blockIdx.x * blockDim.x + threadIdx.x}}\\
    \multicolumn{3}{c}{\code{C[i] = A[i] + B[i]}}
  \end{tabular}
  \caption{网格中的所有线程执行相同的内核代码（依据原书图 2.9 重绘）}
  \label{fig:2-9}
\end{figure}
...
通常建议线程块中的线程总数是 32 的倍数，这是出于硬件效率的考虑；本书后文会
再次讨论这一点。
...
大的电话线路数量；拨打同一区域的号码时只需拨本地号码，偶尔拨打其他区域的号码
...
\begin{figure}[htbp]
  ...
  \begin{tabular}{>{\ttfamily}p{0.20\textwidth} p{0.18\textwidth} p{0.18\textwidth} p{0.28\textwidth}}
    ...
    \bottomrule
  \end{tabular}
  \caption{CUDA C++ 函数声明关键字（依据原书图 2.11 重绘）}
  \label{fig:2-11}
\end{figure}
...
在自己的实现中可采用任一种风格。} 在 CUDA 内核函数中，自动变量对每个线程都是
私有的。
...
\noindent\textbf{译者注：}图 \ref{fig:2-12} 按原书保留了 \code{int} 返回类型，
但这段概要代码没有 \code{return} 语句；图 \ref{fig:2-13} 的完整版本使用
\code{void}，与该函数的行为一致。
...
向量的不同部分，可以按任意顺序执行。程序员不能对执行顺序作任何假设。
...
\begin{figure}[htbp]
  ...
  \end{tabular}
  \caption{CUDA C++ 程序的编译过程概览（依据原书图 2.14 重绘）}
  \label{fig:2-14}
\end{figure}
