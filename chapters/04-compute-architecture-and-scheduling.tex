...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{figs/ch4/4-1.png}
  \caption{CUDA-capable GPU 的架构}
  \label{fig:4-1}
\end{figure}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{figs/ch4/4-2.png}
  \caption{线程块按块分配给流式多处理器}
  \label{fig:4-2}
\end{figure}
...
\code{_} 字符组成；这是 CUDA 中表示内建函数（intrinsic function）的约定（见侧栏：
“内建函数”）。\textbf{当一个线程调用 \code{__syncthreads()} 时，它会停留在调用所在的程序位置，
直到同一线程块中的每个线程都到达该位置}。调用同步函数的线程被称为到达了与该程序
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.72\textwidth]{figs/ch4/4-3.png}
  \caption{屏障同步的执行时序示例}
  \label{fig:4-3}
\end{figure}
...
__global__ void foo_kernel(...) {
  ...
  \end{lstlisting}
  \caption{错误使用 \code{__syncthreads()} 的示例。}
  \label{fig:4-4}
\end{figure}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{figs/ch4/4-5.png}
...
则每个块有 $256/32=8$ 个 warp；SM 中有三个这样的块，因此共有 $8\times3=24$ 个 warp。

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.65\textwidth]{figs/ch4/4-6.png}
  \caption{线程块被划分为 warp 以进行线程调度}
  \label{fig:4-6}
\end{figure}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{figs/ch4/4-7.png}
  \caption{将二维线程放入线性布局}
  \label{fig:4-7}
\end{figure}
...
\footnote{本文用 $T_{z,y,x}$ 表示线程坐标，三个下标依次对应
\code{threadIdx.z}、\code{threadIdx.y} 和 \code{threadIdx.x}；
因此这里的 $2\times8\times4$ 按 $z\times y\times x$ 排列。}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.48\textwidth]{figs/ch4/4-8.png}
  \caption{SM被组织成处理块以进行SIMD。在这个玩具示例中，
  每八个流处理器形成一个处理块。SM由两个处理块组成。}
  \label{fig:4-8}
\end{figure}
...
从系统产生结果。执行程序时，计算机首先把程序及其数据输入内存。

\begin{center}
  \includegraphics[width=0.62\textwidth]{figs/ch4/sidebar-von-neumann.png}
\end{center}
...
\begin{center}
  \includegraphics[width=0.72\textwidth]{figs/ch4/sidebar-simd.png}
\end{center}

所有处理单元都由指令寄存器中的同一条指令控制，
...
路径。在每次通过其中一条路径时，跟随另一条路径的线程不允许生效。
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.72\textwidth]{figs/ch4/4-9.png}
  \caption{warp 在 \code{if-else} 语句处分歧的示例}
  \label{fig:4-9}
\end{figure}
...
分歧也可能出现在其他控制流结构中。图 \ref{fig:4-10} 展示了\textbf{分歧
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.72\textwidth]{figs/ch4/4-10.png}
  \caption{warp 在 \code{for} 循环处分歧的示例}
  \label{fig:4-10}
\end{figure}
...
线程。因此，一个 SM 可以拥有多达16倍于当前一个时钟周期可执行数量的线程。这种线程相对于 SM 的过量配置
