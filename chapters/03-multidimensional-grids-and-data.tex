...
\textbf{在当前 CUDA 系统中，一个线程块的总大小限制为 1024 个线程}。只要线程总数不超过
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.7\textwidth]{figs/ch3/3-1.png}
  \caption{CUDA 多维网格组织示例}
  \label{fig:3-1}
\end{figure}

图 \ref{fig:3-1} 中的网格由 4 个线程块组成，排列成一个 $2\times2$ 数组。
每个线程块使用 $(\code{blockIdx.y},\code{blockIdx.x})$ 标记。例如，块
$(1,0)$ 的 \code{blockIdx.y}=1，\code{blockIdx.x}=0。\textbf{需要注意，块和线程标签
...
线程块，如图 \ref{fig:3-2} 所示。粗线表示线程块边界。阴影区域表示覆盖像素的线程。
每个线程负责处理一个像素，其 y、x 坐标由 \code{blockIdx}、
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\textwidth]{figs/ch3/3-2.png}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{figs/ch3/3-3.png}
  \caption{二维 C++ 数组的行主序布局及其一维索引}
  \label{fig:3-3}
\end{figure}
...
\footnote{\noindent\textbf{译者注：}这里统一采用 ITU-R BT.601 对经过传递函数
编码的 RGB 信号计算亮度信号（luma）的常用系数。不同色彩空间可能采用不同系数；
若要求线性光意义上的相对亮度，应先通过逆传递函数将输入线性化，再使用与该
色彩空间基色相匹配的系数。}
...
void colorToGrayscaleConversion(unsigned char* Pout,
    ...
    if (col < width && row < height) {
        ...
        Pout[grayOffset] = (unsigned char)(0.299f*r + 0.587f*g + 0.114f*b);
    }
}
  \end{lstlisting}
...
（\code{unsigned char}）。以 $62\times76$ 的图片为例，块 $(1,0)$ 中线程 $(0,0)$ 计算得到的
\code{Pout} 像素的一维索引为：
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{figs/ch3/3-5.png}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{figs/ch3/3-6.png}
  \caption{原始图像（左）及其模糊版本（右）}
  \label{fig:3-6}
\end{figure}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{figs/ch3/3-7.png}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.62\textwidth]{figs/ch3/3-9.png}
  \caption{图像边缘附近像素的边界条件}
  \label{fig:3-9}
\end{figure}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.78\textwidth]{figs/ch3/3-10.png}
  \caption{使用多个线程块分块计算矩阵 $P$}
  \label{fig:3-10}
\end{figure}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.75\textwidth]{figs/ch3/3-12.png}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.62\textwidth]{figs/ch3/3-13.png}
  \caption{一个线程块执行矩阵乘法时的动作}
  \label{fig:3-13}
\end{figure}
