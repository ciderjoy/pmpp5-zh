...
- 已完成第 1--24 章及附录 A--C 的翻译并纳入 `main.tex`，覆盖基础概念、并行模式、
  高级模式与应用，以及数值计算、深度学习和 CUDA 内存模型补充材料。
...
- 第 23 章“多 GPU 编程”已完成全章翻译，覆盖 MPI、NCCL、NVSHMEM、域分区、halo 交换、计算通信重叠、CUDA stream/事件、固定页内存和对称堆；图 23.1--23.25 均以 TikZ 或可复制代码/API 清单重绘、重排。
- 第 24 章“总结与未来展望”已完成全章翻译，覆盖目标回顾、GPU 性能演进、超大规模数据中心、并行计算未来趋势及跨行业计算范式转变；图 24.1 性能对比表已用 LaTeX 重绘，并对底本数值之间的算术不一致添加译者注。
- 附录 A“数值计算考量”、附录 B“深度学习基础”和附录 C“CUDA 内存、地址空间与指针”
  已按原书顺序完整翻译；23 幅附录图均以 TikZ、LaTeX 表格或可复制代码重绘，并完成逐页
  底本核对、整书编译和视觉验收。
- 当前译稿使用 Tectonic 编译；原书插图均以中文概念图、LaTeX 表格或可复制代码清单重绘。
