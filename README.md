...
- 已完成第十七章“稀疏矩阵计算”的中文技术讲解，包含 COO、CSR、ELL、混合 ELL--COO、JDS 和 CSC 格式及 SpMV 内核。
- 第一至第十七章已按原书顺序纳入 `main.tex`。当前整书译稿可由 Tectonic 编译生成 PDF，并已完成交叉引用和版面警告检查。
...
当前已纳入第一章至第十七章。后续章节完成后，在 `main.tex` 中加入相应的 `\include` 即可。
